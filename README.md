...
- **tf_prefix**: By default all frame's ids have the same prefix - `camera_`. This allows changing it per camera. UNUSED, see frame_id_prefix. 
- **frame_id_prefix**:  Custom ANYbotics code. By default all frame's ids have the same prefix - `camera_`. This allows changing it per camera. Setting individual frame ids overrides this prefix, i.e. the prefix is only used for default frame_id resolution.
- **base_frame_id**: defines the frame_id all static transformations refers to. Default: `camera_` + `camera_parent` = `camera_camera_parent`
